...
resource "aws_s3_bucket_object" "job_spark" {
  ...
  key    = "s3://datalake-projeto-aplicado/emr-code/pyspark/job_spark.py"
  source = "../job_spark.py"
...
resource "aws_s3_bucket_object" "insere_postgres" {
  bucket = aws_s3_bucket.bucket-pa.id
  key    = "s3://datalake-projeto-aplicado/etl/insert_postgres.py"
...
resource "aws_s3_bucket_object" "insere_elasticsearch" {
  bucket = aws_s3_bucket.bucket-pa.id
  key    = "s3://datalake-projeto-aplicado/etl/insert_elasticsearch.py"
